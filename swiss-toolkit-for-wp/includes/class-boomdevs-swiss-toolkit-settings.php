...
if (!class_exists('BDSTFW_Swiss_Toolkit_Settings')) {
    class BDSTFW_Swiss_Toolkit_Settings
    {
            ...
            Redux::set_section(BDSTFW_Swiss_Toolkit_Settings::$prefix, array(
                ...
                'fields' => array(
                    ...
                    array(
                        ...
                        'type'    => 'switch',
                        'title'   => __('WP Admin Login URL Changer', 'boomdevs-swiss-toolkit'),
                        'subtitle'   => __('Secures WordPress site effortlessly by modify admin login URL to enhance sites security.', 'boomdevs-swiss-toolkit'),
                        'help'  => __('Please be careful with the address', 'boomdevs-swiss-toolkit'),
                        'class' => 'swiss_help_icon bdstfw_switch_toggle',
                        'default' => false,
                    ),
                    array(
                        'id'      => 'boomdevs_swiss_change_login_url_text',
                        'type'    => 'text',
                        'title'   => '<code>' . esc_url(home_url()) . '/</code>',
                        'class'   => 'swiss_change_login_url',
                        'default' => 'login',
                        'required' => array('boomdevs_swiss_change_login_url_switcher', 'equals', 'true'),
                        'validate' => array('str_replace', 'no_special_chars'),
                        'str' => array(
                            'search' => ' ',
                            'replacement' => '-'
                        ),
                        'validate_callback' => [$this, 'login_url_validate_callback']
                    ),
                     ...
                     */
                    array(
                        'id'     => 'boomdevs_swiss_optimizations_switch',
                        'type'   => 'switch',
                        'title'   => esc_html__('Database Optimizations ', 'swiss-toolkit-for-wp'),
                        'subtitle' => esc_html__('Optimize your database by removing unnecessary data and improving performance without the need for additional plugins.', 'swiss-toolkit-for-wp'),
                        'class' => 'bdstfw_switch_toggle',
                        'default' => false
                    ),
                ),
        ...
        /**
         * Login url validation custom callback.
         */
        public function login_url_validate_callback($field, $value, $existing_value)
        {
            $warning = false;
            $error = false;
            ...
            if (true === $error) {
                $field['msg']      = 'Special character not allowed';
                $return['error'] = $field;
            }

            if (true === $warning) {
                $field['msg']      = 'WP-admin is wordpress default admin url, cannot use it';
