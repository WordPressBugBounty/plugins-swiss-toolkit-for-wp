    ...
    class BDSTFW_Swiss_Toolkit_Generate_URL
    {
        ...
        public function render_generate_url_column_content($column, $post_id)
        {
            if ($column === 'generate_url') {
                $custom_data = get_post_meta(intval($post_id), '_bdstfw_encrypted_token', true);
                ...
                $expiration_time = get_post_meta(intval($post_id), '_bdstfw_swiss_expiration_time', true);
                $usage_count = get_post_meta(intval($post_id), '_bdstfw_swiss_usage_count', true);
        ...
        {
            if (isset($_GET['post'])) {
                $token = home_url() . '?sk=' . get_post_meta(intval($_GET['post']), '_bdstfw_encrypted_token', true);
        ...
        public function create_generate_url_metaboxes()
        {
            ...
            if (!$pro_generate_login_settings) {
                $pro_generate_login_settings = array(
                    ...
                    array(
                        'id' => '_swiss_usage_custom_limitation',
                        'type' => 'text',
                        'min' => 1,
                        ...
                        'required' => array('_swiss_usage_limitation', 'equals', 'custom')
                    ),
                );
            }
        ...
        public function save_swiss_generate_url_post($post_id)
        {
            ...
            if (!get_post_meta(intval($post_id), '_bdstfw_encrypted_token', true)) {
                update_post_meta(intval($post_id), '_bdstfw_encrypted_token', $encrypted_token);
            }

            // Update user ID
            update_post_meta(intval($post_id), '_bdstfw_current_login_userId', get_current_user_id());
            ...
            $expiration_limitation = isset($_POST['swiss-toolkit-for-wp']['_swiss_usage_limitation']) ? sanitize_text_field($_POST['swiss-toolkit-for-wp']['_swiss_usage_limitation']) : '';
            ...
            if (!$pro_global_settings) {
                ...
                update_post_meta(intval($post_id), '_bdstfw_swiss_expiration_time', $expiration_time);
                update_post_meta(intval($post_id), 'bdstfw_swiss_usage_limitation', 3);
            } else {
                update_post_meta(intval($post_id), '_bdstfw_swiss_expiration_time', $expiration_time);

                if ($expiration_limitation === 'custom') {
                    $custom_expiration_limit = isset($_POST['swiss-toolkit-for-wp']['_swiss_usage_custom_limitation']) ? sanitize_text_field($_POST['swiss-toolkit-for-wp']['_swiss_usage_custom_limitation']) : '';
            ...
            if ($usage_count === '') {
                ...
                $usage_count = 0;
                update_post_meta(intval($post_id), '_bdstfw_swiss_usage_count', $usage_count);
            }
